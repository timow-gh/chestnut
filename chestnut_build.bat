...
set BUILD_DIR=%REPOSITORY_PARENT_PATH%\chestnut_build
set SOURCE_DIR=.\
set INSTALL_DIR=%REPOSITORY_PARENT_PATH%\chestnut_install
...
cmake   -B %BUILD_DIR% -S %SOURCE_DIR% -G %CMAKE_GENERATOR% -A x64 ^
        ...
        -Dchestnut_BUILD_TESTS=ON ^
        -Dchestnut_BUILD_BENCMARKS=ON ^
        -Dchestnut_WARNINGS_AS_ERRORS=ON ^
        -Dchestnut_BUILD_PACKAGE=ON
